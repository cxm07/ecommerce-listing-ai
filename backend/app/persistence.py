...
class PostgresRepository:
    ...
    def list_generated_content(self, task_id: UUID) -> list[GeneratedContent]:
        ...
        return [GeneratedContent(r["id"],r["task_id"],r["product_id"],r["title"],r["selling_points"],r["unsupported_claims"],r["model_metadata"],r["created_at"].isoformat(),r["version"]) for r in rows]
    ...
    def add_content(self, x: GeneratedContent) -> None: self._cursor().execute("insert into public.generated_contents(id,task_id,product_id,version,title,selling_points,unsupported_claims,model_metadata,created_at) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)",(x.id,x.task_id,x.product_id,x.version,x.title,Jsonb(x.selling_points),Jsonb(x.unsupported_claims),Jsonb(x.model_metadata),x.created_at))
    ...
    def add_audit(self, x: AuditLog) -> None: self._cursor().execute("insert into public.audit_logs(id,task_id,actor_id,action,source_ref,created_at) values(%s,%s,%s,%s,%s,%s)",(x.id,x.task_id,self._actor(),x.action,Jsonb(x.source_ref) if x.source_ref is not None else None,x.created_at))
